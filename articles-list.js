...
function filterArticlesByStatus(articles) {
    ...
    return articles.filter(article => {
        ...
        // SHOW articles that are:
        // 1. Published
        // 2. Approved (ready for publishing)
        // 3. Have no status (old articles before system)
...
function loadAllArticles() {
    ...
    db.ref('articles').orderByChild('date').once('value')
        .then((snapshot) => {
           ...
           // Get ALL filtered articles, not just 6
initialArticles = filterArticlesByStatus(allArticles);  // Shows all articles
            ...
            // Display only the filtered articles WITH IMAGES
            displayArticles(initialArticles, '', true);  // Use initialArticles which now has ALL filtered articles
        })
        .catch((error) => {
...
function displayArticles(articles, searchTerm = '', showImages = true) {
    ...
    // ✅ ADD THIS FILTER - Only show published/approved articles
    ...
    // Use filteredArticles for everything below
